...
class MessageDuplexHost extends EventEmitter {
    ...
    handleNewConnection(channel) {
        ...
        if(extensionID !== extensionizer.runtime.id) {
            channel.disconnect();
            return console.warn(`Dropped unsolicited connection from ${ extensionID }`);
        }
        ...
        // console.info(`New connection ${ name }:${ uuid } acquired`, channel);
        ...
        channel.onDisconnect.addListener(() => {
            // console.info(`Connection ${ name }:${ uuid } disconnected`);
    ...
    handleMessage(source, message) {
        ...
        if(source == 'tab' && ![ 'tabRequest' ].includes(action))
            return console.error(`Droping unauthorized tab request: ${ action }`, data);
