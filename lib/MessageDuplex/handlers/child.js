...
class MessageDuplexChild extends EventEmitter {
    ...
    connectToHost() {
        ...
        this.disconnectListener = this.channel.onDisconnect.addListener(() => {
            ...
            console.error('Lost connection to MessageDuplexHost:', error);
    ...
    resetGovernor() {
        ...
        this.governor = {
            ...
            queue: [],
            reconnect: () => {
                console.warn('MessageDuplexChild requested reconnect');
            }
        };
    ...
    handleMessage({ action, data, messageID, noAck = false }) {
        // console.info('Received new message', { action, data, messageID });
    ...
    handleReply({ messageID, error, res }) {
        ...
        // return console.info(`Cannot find promise for message ${ messageID }`);
